...
/*
  ...
  Args:
    b      : batch size
    n      : number of points
    r      : voxel resolution (points -> [r, r, r] voxel)
    r2     : = r * r
...
__global__ void grid_stats_kernel(int b, int n, int r, int r2, int r3,
  ...
  ind += batch_index * n;
  cnt += batch_index * r3; // at most r3 points in each voxel

  for (int i = index; i < n; i += stride) {
    // amazing, flattened operation is such simple.
...
__global__ void avg_voxelize_kernel(int b, int c, int n, int s,
  ...
  for (int i = index; i < n; i += stride) {
    ...
    int cur_cnt = cnt[pos];
    if (cur_cnt > 0) {
      // averaged on all points in this voxel
...
void avg_voxelize(int b, int c, int n, int r, int r2, int r3, const int *coords,
  ...
  grid_stats_kernel<<<b, optimal_num_threads(n)>>>(b, n, r, r2, r3, coords, ind, cnt);
  avg_voxelize_kernel<<<b, optimal_num_threads(n)>>>(b, c, n, r3, ind, cnt, feat, out);
  CUDA_CHECK_ERRORS();
}

void avg_voxelize_grad(int b, int c, int n, int s, const int *ind,
                       const int *cnt, const float *grad_y, float *grad_x) {
  avg_voxelize_grad_kernel<<<b, optimal_num_threads(n)>>>(b, c, n, s, ind, cnt, grad_y, grad_x);
  CUDA_CHECK_ERRORS();
}
