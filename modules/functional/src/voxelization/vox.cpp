...
/*
  Function: trilinear pool voxelization (forward)
  Args:
    features: features, FloatTensor[b, c, n]
    coords  : coords of each point, IntTensor[b, 3, n]
    resolution : voxel resolution
  Return:
    out : outputs, FloatTensor[b, c, s], s = r ** 3
    ind : voxel index of each point, IntTensor[b, 8, n]
    wgt : weights, FlaotTensor[b, 8, n] 
    cnt : #points in each voxel index, IntTensor[b, s]
*/
std::vector<at::Tensor> trilinear_voxelize_forward(const at::Tensor features,
  ...
  at::Tensor ind = torch::zeros({b, 8, n}, at::device(features.device()).dtype(at::ScalarType::Int));
  at::Tensor wgt = torch::zeros({b, 8, n}, at::device(features.device()).dtype(at::ScalarType::Float));
  at::Tensor out = torch::zeros({b, c, r3}, at::device(features.device()).dtype(at::ScalarType::Float));
  at::Tensor cnt = torch::zeros({b, r3}, at::device(features.device()).dtype(at::ScalarType::Int));

  trilinear_voxelize(b, c, n, r, r2, r3, coords.data_ptr<float>(), features.data_ptr<float>(), 
               ind.data_ptr<int>(), wgt.data_ptr<float>(),
               cnt.data_ptr<int>(), out.data_ptr<float>());

  return {out, ind, wgt, cnt};
}
...
/*
  ...
  Args:
    grad_y : grad outputs, FloatTensor[b, c, s]
    indices: voxel index of each point, IntTensor[b, 8, n]
    weights: FloatTensor[b, 8, n]
    cnt    : #points in each voxel index, IntTensor[b, s]
  Return:
    grad_x : grad inputs, FloatTensor[b, c, n]
*/
at::Tensor trilinear_voxelize_backward(const at::Tensor grad_y,
  ...
  trilinear_voxelize_grad(b, c, n, s, indices.data_ptr<int>(), weights.data_ptr<float>(), cnt.data_ptr<int>(),
                    grad_y.data_ptr<float>(), grad_x.data_ptr<float>());
  return grad_x;
}
